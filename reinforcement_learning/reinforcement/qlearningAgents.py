...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        bestAction = max(action_Qvalues, key=lambda x: x[1])[0]
        return bestAction
        # util.raiseNotDefined()
    ...
    def getAction(self, state):
        ...
        if len(legalActions) == 0:
            return action
        if util.flipCoin(self.epsilon):  # randomly pick one action
            action = random.choice(legalActions) 
        else:  # use be best action in current Qtable
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.Qtable[(state, action)] = (1-self.alpha)*self.getQValue(state, action) + self.alpha*(reward + self.discount*self.getValue(nextState))
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for nextAction in nextActions:
            ...
            Qvalues.append(Qvalue)
        maxQvalue = 0 if len(nextActions) == 0 else max(Qvalues)
